...
def _resolve_stl(path) -> str:
    """Find an STL that was named without a full path.

    Callers here name supplied geometry bare, for example
    ``Cockpit_V1_3.stl``, which only resolves when the working directory
    happens to hold it. In this repository the supplied STLs live in exactly
    one place, ``source_documents/`` at the repository root, because they are
    read by the CAD scripts as well and must not be duplicated.

    So: use the path as given when it exists, otherwise look in the known
    locations. Returns the original path unchanged when nothing matches, so
    the caller still raises its own FileNotFoundError with the name the user
    typed rather than a confusing rewritten one.
    """
    if os.path.isfile(path):
        return path

    name = os.path.basename(str(path))
    here = os.path.dirname(os.path.abspath(__file__))   # powertrain/python/volare
    project_root = os.path.dirname(os.path.dirname(here))   # powertrain
    repo_root = os.path.dirname(project_root)

    candidates = [
        os.path.join(repo_root, "source_documents", name),
        os.path.join(project_root, "04_data", "geometry", name),
        # volare_params.json spells this one with spaces and a dot.
        os.path.join(project_root, "04_data", "geometry",
                     name.replace("_V1_3", " V1.3").replace("_", " ")),
    ]
...
def load_stl(path) -> np.ndarray:
    ...
    A bare filename is resolved against the repository's ``source_documents/``
    directory; see :func:`_resolve_stl`.
    """
    path = _resolve_stl(path)
